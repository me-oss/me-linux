...
#include <asm/arch/w55fa93_nand.h>
#include <asm/arch/gnand/GNAND.h>
#include <asm/arch/NandDrv.h>
...
#define ONE_NAND
#elif defined (CONFIG_W55FA93_NAND_BOTH)
//#error ha ha ha
#define ONE_NAND
#define TWO_NAND
#else
...
#define sd_inl(addr)				  	inl(addr)
#define sd_outl(data, addr)			outl(data, addr)
...
static int nand_alloc0 = 0, nand_alloc1 = 0;
...
static irqreturn_t fmi_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
...
//	 outpw(REG_SMTCR, 0x10305);     //  AHB:cpu=1:1
        ...
        if (down_interruptible(&dmac_sem))
                return;
        while (inl(REG_DMACCSR)&DMACCSR_FMI_BUSY); //Wait IP finished... for safe
...
static int nand_scsi_read(struct scsi_cmnd  *cmd , struct nand_hostdata *dev)
{
        ...
        int volatile DMAoffset = 0, status = 0, ii, jj;
        int curDMAAddr;
...
//	printk("nand_scsi_read--Read from = %d(lba),  count = %d   \n", lba, count);
...
_gnand_read_retry0_:
                ...
                MSG2("count  %d\n",count);
                curDMAAddr = (unsigned int )page_address(curList->page) + curList->offset + nand_host.curOffset;
                ...
                while ((curList->length)<(count*512)) {
                        ...
                        curDMAAddr =
                                (unsigned int )kmap_atomic(curList->page, KM_USER0) +
                                curList->offset + nand_host.curOffset;
...
_gnand_read_retry1_:
        ...
        //       printk("GNAND_read_1, status = %2x \n", status);                                
                ...
                MSG2("curList->length  %d\n",(curList->length)/512);
                MSG2("count  %d\n",count);
                curDMAAddr = (unsigned int )page_address(curList->page) + curList->offset + nand_host1.curOffset;
                ...
                while ((curList->length)<(count*512)) {
                        ...
                        curDMAAddr =
                                (unsigned int )kmap_atomic(curList->page, KM_USER0) +
                                curList->offset + nand_host1.curOffset;
...
static int sd_add_card(void)
{
        ...
        if ((sd_add == 0 )&&(nand_flagSetting.curCard == 0)) { //add card 0
                ...
                sprintf(nand_host.dev.bus_id, "card0");
        ...
        if ((sd_add1 == 0 )&&(nand_flagSetting.curCard == 1)) { //add card 1
                ...
                sprintf(nand_host1.dev.bus_id, "card1");
...
static void sd_del_card(void)
{
        ...
        if ((sd_add1 )&&(nand_flagSetting.sdRemove == 1)) {
              //  printk("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF\n");
                if(ptNDisk1 != 0) {
...
//		outpw(REG_AHBCLK, inpw(REG_AHBCLK) | SD_CKE);
        ...
        if (request_irq(IRQ_SIC, fmi_interrupt, SA_SHIRQ|SA_INTERRUPT, "W55FA93_NAND", (void*)1)) { //SA_INTERRUPT
