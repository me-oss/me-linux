...
#include <asm/arch/gnand/GNAND_global.h>
...
int fmiSMCheckRB(FMI_SM_INFO_T *pSM)
{
	...
	#if defined(CONFIG_W55FA93_TWO_RB_PINS)        	
				if(pSM == pSM0)
				{
...
//                printk("tempID[0]= 0x[%x]\n", tempID[0]);
//                printk("tempID[1]= 0x[%x]\n", tempID[1]);
//                printk("tempID[2]= 0x[%x]\n", tempID[2]);
//                printk("tempID[3]= 0x[%x]\n", tempID[3]);                                                
							    ...
							    printk("register REG_SMISR = %4x \n ", inpw(REG_SMISR));                        
						    		...
						    		printk("SMRA contents: \n ");               
						                for(ii=0; ii<4; ii+=0x10)
						                {
											for(jj=0; jj<0x10; jj++)
											{
						                		printk("0x%2x ", inl(REG_SMRA_0+ii*0x10+jj));                                		
											}						                	
						 	              		printk("\n");                                		                	
										}                	
...
{
        //printk("sicSMpwrite  PBA=%d  page=%d buff=%x\n",PBA,page,buff);
...
//		printk("sicSMpwrite NandPort=%d, PBA=%2x, pageNo=%2x, buffer=%4x\n",NandPort, PBA, pageNo, (u32)buff);                
