...
{
	outl(  (inl(REG_HC_RH_OP_MODE) & ~(DISPRT2 | DISPRT1)) |
...
static void usbh_LikeModePort(unsigned int u32HlmPort)
{
	if(u32HlmPort==1)
	{
		outl ( (inl(REG_GPBFUN) & ~(MF_GPB1 | MF_GPB0)) | 0x05, REG_GPBFUN); 
	}
	else if(u32HlmPort==2)
	{
		outl ( (inl(REG_GPAFUN) & ~(MF_GPA4 | MF_GPA3)) | (0x0A<<6), REG_GPAFUN); 
	}
}
...
static int  get_pll()
{
	...
	DBG_PRINTF("%s\n",__FUNCTION__);	
	u32PorSetting = (inl(REG_CHIPCFG) & COPMODE)>>2;
	if(u32PorSetting==2)
	{
			  ...
			  struct platform_device *pdev)
{
	 ...
	 * Config GPIO
	 */
	//outl(inl(REG_GPIOA_OMD) & ~0x01, REG_GPIOA_OMD);	// GPIOA0 input mode
	//outl(inl(REG_GPIOB_OMD) | 0x5000, REG_GPIOB_OMD);	// GPIOB12,B14 output mode
	//outl(inl(REG_GPIOB_DOUT) | 0x5000, REG_GPIOB_DOUT);	// GPIOB12,B14 output high

	/* 
	 * Config multi-function
	 */	
	// No over currnet pin assigned on EV board
	// outl((inl(REG_PINFUN) & 0x00ffffff)/* | 0x11000000 */, REG_PINFUN);
	...
	outl( (inl(REG_CLKDIV2) & ~(USB_N1 | USB_S | USB_N0)) | 
					( ((div1 << 8) | (3<<3)) | (div0) ), 
					REG_CLKDIV2);
...
//#endif
	...
	outl(inl(REG_AHBCLK) | USBH_CKE, 
			REG_AHBCLK);
	/* reset USB host */
	outl( inl(REG_AHBIPRST) | UHCRST, 
			REG_AHBIPRST);
	outl( inl(REG_AHBIPRST) & ~UHCRST, 
			REG_AHBIPRST);
	...
	// outl(0x08, W55FA93_VA_USBH_BASE + 0x204);
...
static int __init ohci_hcd_w55fa93_init(void)
{
	DBG_PRINTF("%s\n",__FUNCTION__);
	...
	printk("CONFIG_W55FA93_USB_HOST_LIKE_PORT2\n");
	usbh_LikeModePort(2);				/* GPA3 and GPA4 from port 2*/
#endif

	//printk("ohci_hcd_w55fa93_init() %08x\n ", (int)&ohci_hcd_w55fa93_driver);
	return platform_driver_register(&ohci_hcd_w55fa93_driver);
}

static void __exit ohci_hcd_w55fa93_cleanup(void)
{
	DBG_PRINTF("%s\n",__FUNCTION__);	
	platform_driver_unregister(&ohci_hcd_w55fa93_driver);
}
module_init(ohci_hcd_w55fa93_init);
module_exit(ohci_hcd_w55fa93_cleanup);
