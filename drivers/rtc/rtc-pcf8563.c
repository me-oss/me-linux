...
static int pcf8563_get_datetime(struct i2c_client *client, struct rtc_time *tm, struct rtc_wkalrm *alrm)
{
	...
	if (alrm) {
		printk("%s: alarm raw data is min=%02x, hr=%02x, "
		"mday=%02x, wday=%02x\n",
		__FUNCTION__,
		buf[9], buf[10], buf[11], buf[12]);
	}
...
#if 0
	printk( "%s: %s tm is secs=%d, mins=%d, hours=%d, "
		"mday=%d, mon=%d, year=%d, wday=%d\n",
		__FUNCTION__,
		alrm ? "alarm" : "time",
		tm->tm_sec, tm->tm_min, tm->tm_hour,
...
#if 0
	printk( "%s: %s secs=%d, mins=%d, hours=%d, "
		"mday=%d, mon=%d, year=%d, wday=%d\n",
		__FUNCTION__,
		alrm ? "alarm" : "time",
		tm->tm_sec, tm->tm_min, tm->tm_hour,
...
static int pcf8563_set_alarm(struct i2c_client *client, struct rtc_wkalrm *alrm)
{
	...
	//printk("\nbuf data: min[%x], hr[%x], mday[%x], wday[%x]\n\n", buf[1], buf[2], buf[3], buf[4]);
...
static int pcf8563_set_rtc_ioctl(struct i2c_client *client, unsigned int cmd, unsigned long arg)
{
	int err;
	unsigned char buf[2];
	...
	case RTC_RD_AF:
		...
		if (regbuf[PCF8563_REG_ST2] & PCF8563_ST2_AF)
			return 1;
		else
			return 0;
...
{
	return pcf8563_set_rtc_ioctl(to_i2c_client(dev), cmd, arg);
}
