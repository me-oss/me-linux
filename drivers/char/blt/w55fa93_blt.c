...
static int w55fa93_blt_ioctl(struct inode *inode, struct file *file, unsigned int cmd, void *arg)
{
	...
	int ret = 0, ret1, i;
	unsigned int RGB565TransparentKey;
...
//printk("u32FrameBufAddr = 0x%p\n", blitop.dest.u32FrameBufAddr);
	...
	case BLT_EDMA_SINGLE:
		...
		ret = w55fa93_edma_setup_handlers(0, eDRVEDMA_BLKD, blt_edma_irq_handler, NULL);
		if (ret < 0) {
...
{
	dev_t dev = MKDEV(BLT_MAJOR, BLT_MINOR);

	if (register_chrdev_region(dev, 1, DRIVER_NAME)) {
...
static void __exit w55fa93_blt_cleanup(void)
{
	dev_t dev = MKDEV(BLT_MAJOR, BLT_MINOR);
...
module_exit(w55fa93_blt_cleanup);
