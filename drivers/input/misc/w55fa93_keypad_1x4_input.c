...
static void read_key(unsigned long arg)
{
        ...
        new_key = inl(REG_GPIOA_PIN) & 0x3C;
...
//	  printk("->0\n");
	...
	new_key = (~(inl(REG_GPIOA_PIN)) & 0x3C) >> 2;
...
static irqreturn_t w55fa93_kpd_irq(int irq, void *dev_id, struct pt_regs *regs) {
        ...
        // clear source
        outl(src & 0x000003C, REG_IRQTGSRC0);
        return IRQ_HANDLED;
}
...
int w55fa93_kpd_open(struct input_dev *dev) {
        ...
        outl(inl(REG_IRQTGSRC0) & 0x000003C, REG_IRQTGSRC0); // clear source
	...
	outl((inl(REG_IRQENGPA)& ~(0x003C0000)) | 0x000003C, REG_IRQENGPA); 
...
void w55fa93_kpd_close(struct input_dev *dev) {
		...
		outl((inl(REG_IRQENGPA)& ~(0x003C003C)), REG_IRQENGPA); 
                del_timer(&kpd_timer);
...
static int __init w55fa93_kpd_reg(void) {
        ...
        // init GPIO
        // PORTA[2-5]
        outl(inl(REG_GPIOA_OMD) & ~((1 << 2) | (1 << 3) | (1 << 4) | (1 << 5)), REG_GPIOA_OMD); // input
        outl(inl(REG_GPIOA_PUEN) | ((1 << 2) | (1 << 3) | (1 << 4) | (1 << 5)), REG_GPIOA_PUEN); // pull-up
        outl(inl(REG_IRQSRCGPA) & ~(0xFF0), REG_IRQSRCGPA); // GPA[2~5] as nIRQ0 source
	outl((inl(REG_IRQENGPA)& ~(0x003C0000)) | 0x000003C, REG_IRQENGPA); // falling edge trigger
