...
#endif
#define MVFP_MIRROR		0x20	//0b00000100
#define MVFP_MIRROR_FLIPUP	0x30
...
__u8 GC0308_TABLE[][2] =
{
	...
	// GC Recommended on 25fps
	{0x01 , 0x6a},		// 0xfa ( Hsync Width )
	{0x02 , 0x70},		// 0x70 ( Vsync Width )
	{0x0f , 0x00},		// 0x01

	//{0x01 , 0xfa},		// 0xfa ( Hsync Width )
	//{0x02 , 0x70},		// 0x70 ( Vsync Width )
	//{0x0f , 0x01},		// 0x01
...
//    {0x02,0x24},
	...
	//{0xe9 , 0xf0},
	//{0xea , 0x0d},   //exp level 4  < 4.00fps  ( Better NightVision )
	//{0xeb , 0x7a},
	...
	{0x20 , 0xff},		// 0xff,0xef=Disable Edge Enhancement
	{0x21 , 0xe8},		// default=0xf8
	{0x22 , 0x57},
...
//	{0x26 , 0x3e},		// 0x3f // 0x02 // 0x3e
	...
	{0xb3 , 0x40},		// 0x34,0x38,0x3d,0x40,0x44,0x48,0x50

	{0xb5 , 0x00},		// Brightness(Default=0)   0xd0,0xe0,0xf0,0x00,0x20,0x30,0x40
	...
	//-------------H_V_Switch(4)---------------//
	{0x14 , 0x13},		// 0x13 // jason0003 default normal is flip on
...
//		{0xb3,0x40},	{0xb4,0x80}, {0xba,0x00}, {0xbb,0x00},
...
{0xff,0xff}
    ...
    {0x1e,0x60},
    {0x1f,0x16},
    {0x20,0xef},		// default=0xff  0xef=disable edge enhancement
    {0x21,0xf8},
    {0x22,0x57},
...
//	udelay(100);
	...
	// debug
	//printk("uDeviceID = 0x%x\n", uDeviceID);

	// auto detect
	//sensor_id=DrvVideoIn_I2cReadOV( 0x42, 0x00 );
	sensor_id=I2C_Read_8bitSlaveAddr_8bitReg_8bitData(0x42,0x00);
	printk("Sensor ID=%x\n",sensor_id);
	if ( sensor_id==0x9b ) printk("Init GC0308\n");
	else printk("Init OV7675\n");
...
// Use GC0308
	...
	for(i=0;;i++)
    	{
	    	if ( GC0308_TABLE[i][0]==0xff ) break;
	        GC0308_WriteReg(GC0308_TABLE[i][0],GC0308_TABLE[i][1]);
    	}
	udelay(100);
    	...
    	if ( 1 ) {
    	// 50Hz ( For 220v )
	printk("Anti-flicking=50Hz\n");
	GC0308_WriteReg(0x01,0x6a);
	GC0308_WriteReg(0x02,0x70);
	GC0308_WriteReg(0x0f,0x00);
	GC0308_WriteReg(0xe3,0x96);   // step
	GC0308_WriteReg(0xe4,0x02);
	GC0308_WriteReg(0xe5,0x58);
	GC0308_WriteReg(0xe6,0x02);
	GC0308_WriteReg(0xe7,0x58);
	GC0308_WriteReg(0xe8,0x02);
	GC0308_WriteReg(0xe9,0x58);
	GC0308_WriteReg(0xea,0x0a);	// 0x09
	GC0308_WriteReg(0xeb,0x60);
	}
	else  // 60Hz ( For 110v )
	{
	printk("Anti-flicking=60Hz\n");
	GC0308_WriteReg(0x01,0x6a);
	GC0308_WriteReg(0x02,0x89);
	GC0308_WriteReg(0x0f,0x00);
	GC0308_WriteReg(0xe3,0x7d);   // step
	GC0308_WriteReg(0xe4,0x02);
	GC0308_WriteReg(0xe5,0x71);
	GC0308_WriteReg(0xe6,0x02);
	GC0308_WriteReg(0xe7,0x71);
	GC0308_WriteReg(0xe8,0x02);
	GC0308_WriteReg(0xe9,0x71);
	GC0308_WriteReg(0xea,0x0a);			// 0x09
	GC0308_WriteReg(0xeb,0xc4);
	}
...
void GC0308_WriteReg(UINT8 address,UINT8 byte ){
	udelay(10);
	//DrvVideoIn_I2cWriteOV(0x42, address, byte);
	if ( I2C_Write_8bitSlaveAddr_8bitReg_8bitData(0x42, address, byte)==FALSE ) printk("I2C Err\n");
}
void go_bw()
{
	UINT8 temp_reg;
	/*
	GC0308_WriteReg( 0x23,0x02 );
	GC0308_WriteReg( 0x2d,0x0a );
	GC0308_WriteReg( 0x20,0x7f );
	GC0308_WriteReg( 0xd2,0x90 );
	GC0308_WriteReg( 0x73,0x00 );
	GC0308_WriteReg( 0xb3,0x40 );
	GC0308_WriteReg( 0xb4,0x80 );
	GC0308_WriteReg( 0xba,0x00 );
	GC0308_WriteReg( 0xbb,0x00 );
	*/
	// Night Vision
	//temp_reg = DrvVideoIn_I2cReadOV( 0x42, 0x20 );
	temp_reg=I2C_Read_8bitSlaveAddr_8bitReg_8bitData(0x42,0x20);

	GC0308_WriteReg(0xec  ,0x30);
	GC0308_WriteReg(0x20  ,temp_reg&0x5f);   // close cc
	GC0308_WriteReg(0x3c  ,0x08);
	GC0308_WriteReg(0x3d  ,0x08);
	GC0308_WriteReg(0x3e  ,0x08);
	GC0308_WriteReg(0x3f  ,0x08);
}
void go_color()
{
	UINT8 temp_reg;
	GC0308_WriteReg( 0x23,0x00 );
	GC0308_WriteReg( 0x2d,0x0a );
	GC0308_WriteReg( 0x20,0x7f );
	GC0308_WriteReg( 0xd2,0x90 );
	GC0308_WriteReg( 0x73,0x00 );
	GC0308_WriteReg( 0x77,0x54 );
	GC0308_WriteReg( 0xb3,0x40 );
	GC0308_WriteReg( 0xb4,0x80 );
	GC0308_WriteReg( 0xba,0x00 );
	GC0308_WriteReg( 0xbb,0x00 );
	...
	GC0308_WriteReg(0x20 ,temp_reg|0x20);
	GC0308_WriteReg(0x3c ,0x02);
	GC0308_WriteReg(0x3d ,0x02);
	GC0308_WriteReg(0x3e ,0x02);
	GC0308_WriteReg(0x3f ,0x02);
}
...
//Brightness={255,223,192,159,0,31,63,95,127};
//Contrastg={0,32,64,96,128,160,192,224,255};

//0xb3 0x34,0x38,0x3d,0x40,0x44,0x48,0x50
UINT8 contrast_mapper[][2]={
	{ 0, 0x34},
	{ 32,0x36 },
	{ 64,0x38 },
	{ 96,0x3d },
...
void mlsSensorOV76xxSetContrast(int value)
{
    ...
    {
    	for(i=0;i<9;i++) {
    		if ( value==contrast_mapper[i][0] ) { 
    			GC0308_WriteReg(0xb3,contrast_mapper[i][1]);
    			break;
    		}
    	}
    	if ( i==9 ) GC0308_WriteReg(0xb3,0x40);		// normal
		return;
    }
    else
    {
...
void mlsSensorOV76xxSetBrightness(int value)
{
    ...
    {
	for(i=0;i<9;i++) {
		if ( value==brightness_mapper[i][0] ) { 
			GC0308_WriteReg(0xb5,brightness_mapper[i][1]);
			break;
		}
	}
	   if ( i==9 ) GC0308_WriteReg(0xb5,0);		// normal
	   return;
    }
    else
    {
...
void mlsSensorOV76xxFlipup()
{
	...
	{
	    UINT8 temp_reg;
	    temp_reg=I2C_Read_8bitSlaveAddr_8bitReg_8bitData(0x42,0x14);
	    if ((temp_reg & 0x03) == 0x03) // NORMAL NON FLIPUP
	    {
		temp_reg = temp_reg & ~(0x03);
		
	    }
	    else  // FLIPUP GOTO NORMAL
	    {
		temp_reg = temp_reg | 0x03;
	    }
	    GC0308_WriteReg(0x14 ,temp_reg);
	}
	else
	{
	    regVal = I2C_Read_8bitSlaveAddr_8bitReg_8bitData(0x42, 0x1E);
	    printk("value of 0x1E from device:%x\t");
	    if((regVal & MVFP_MIRROR_FLIPUP) == MVFP_MIRROR_FLIPUP)
	    {
		regVal = regVal & ~MVFP_MIRROR_FLIPUP;
	    }
	    else
	    {
		regVal = regVal | MVFP_MIRROR_FLIPUP;
...
int mlsSensorOV76xxCheckFlipup()
{
	//mlsdev008 8/12/2011
	char regVal;
	if (sensor_id == 0x9b)
	{
	    regVal = I2C_Read_8bitSlaveAddr_8bitReg_8bitData(0x42, 0x14);
	    printk("value of 0x1E from device:%x\t");
	    if((regVal & 0x03) == 0x03)
	    {
		return 1;  // NORMAL // jason0003
	    }
	    else
	    {
		return 0;  // FLIP // jason0003
	    }
