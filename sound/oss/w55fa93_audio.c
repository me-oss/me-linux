...
#define RECBUF_GETALL	1110 // already defined in soundcard.h
...
// Stephen - Enable Short Delay Recording
#define CONFIG_NORECINT
// Stephen - Recording with Interpolation Filter
#define	CONFIG_REC_INTERP
//#define ADC_DEBUG
//#define AUDIOLOSS_DEBUG

#ifdef CONFIG_REC_INTERP
unsigned int startdmabuf, stopdmabuf;
unsigned short vdmaptr, roffptr;
#endif

#ifdef CONFIG_NORECINT
#define	REC_SAMPLE	100
static struct timer_list adc_timer;
unsigned char led_state=0;
signed short *rec_base;
unsigned long rec_offset=0;
signed short rec_buf[505];
unsigned int rec_cnt505=0;

int start_adc_offset=-1;
//int stop_adc_offset=-1;
signed short flat_adc_val = 0;
unsigned int isAdcRead=0;
#define OFFSET_AFTER_ADC_READ 16
#define SPIKE_WIDTH	8
#define SPIKE_DETECTION_WIDTH 70
/*
unsigned int stop_adc_proc=0;

struct semaphore adc_read_sem;
*/
...
static int fragment_size = 1 << 12, max_fragments = 2;  // 13 => 8k + 8k
static int r_fragment_size = 1 << 12, r_max_fragments = 2;  	// 13 => 8k + 8k (for recording)
static int max_fragment_size = 1 << 12;
...
static int record_callback(UINT32 uAddr, UINT32 uLen)
{
	int i = 1;
	int k=0,j=0;
	signed short sampleval = 0;
	signed short *tmpptr, *endptr;
	signed short *spikeptr, *prevspikeptr, *nextspikeptr;
	...
	if (isAdcRead != 0)  // its time to update the buffer
	{
		printk("Tested: i=%d - Startdmabuf = %d\n",i,startdmabuf);
		if (((startdmabuf >= 4096) && (i==1)) || ((startdmabuf < 4096) && (i==0)))
		{
			// do it update the correct value to the buffer
			//tmpptr = (signed short*) (dma_buf + startdmabuf);
			//endptr = (signed short*) (dma_buf + (i+1)*r_fragment_size);
			tmpptr = endptr = NULL;
			j = startdmabuf;
			k = 0;
			prevspikeptr = spikeptr = nextspikeptr = NULL;
			while((j < ((i+1)*r_fragment_size - SPIKE_WIDTH -4 )) && (k<SPIKE_DETECTION_WIDTH) &&(tmpptr==NULL))
			{
				prevspikeptr = (signed short*) (dma_buf + j);
				spikeptr = (signed short*) (dma_buf + j + 2);
				//nextspikeptr = (signed short*) (dma_buf + j + 4);
				if ((*spikeptr < 0) && ((*spikeptr - *prevspikeptr) < -2000))
				{
				    int l=0;
				    tmpptr = spikeptr;
				    for (l=1;l<SPIKE_WIDTH;i++)
				    {
					if ((*tmpptr - *(tmpptr+1)) < -2000)
					{
					    nextspikeptr = tmpptr;
					    break;
					}
				    ...
				    if (nextspikeptr != NULL)
				    {
					tmpptr = prevspikeptr;
					endptr = nextspikeptr;
					printk("Found Spike (%d - %d)\n",((int)tmpptr - audio_dev.record_buf_addr),((int)endptr - audio_dev.record_buf_addr));
				    }
				    else
				    {
					tmpptr = endptr = NULL;
		...
		{
			printk("Over need to wait next step\n");
		}
	}
	...
	if ( audio_dev.record_half_buf[i ^ 1].ptr == 0) {	/* last block wan't take off , user may stop record */
		printk("last block wan't take off , user may stop record");
...
#ifdef CONFIG_NORECINT
static void adc_get_data(unsigned long dummy)
{
	unsigned int remaining=0;
	static int AdcCount = 0;
  	outl((1 << 14 ), REG_AIC_MDCR);
    	disable_irq(14);

	#if defined(CONFIG_SPU_BUF_8K)

	// Stephen
	// If have more than 16 samples, copy to rec__buf until 505 samples is done
	remaining = inl(REG_VDMA_CBCR+edma_channel*0x100);
	//printk("%d\n",remaining);
	vdmaptr = (8192 - remaining)>>1;
	remaining = (vdmaptr > rec_offset)? (vdmaptr - rec_offset): (4096 - rec_offset + vdmaptr);   // 4096 samples only
	//if (  (remaining > 16)  && (rec_cnt505<505) )
	if ( isAdcRead != 0)
	{
	    flush_cache_all();
	}
	
	while (  (remaining > 16)  && (rec_cnt505<505) )
	{
		if (isAdcRead == 0)
		{
		    rec_buf[rec_cnt505] = rec_base[rec_offset];
		}
		else if (isAdcRead == 1)
		{
		    if (rec_offset == start_adc_offset)
		    {
			flat_adc_val = rec_base[rec_offset];
			isAdcRead = 2;
		    }
		...
		if (isAdcRead == 2)
		{
		    rec_buf[rec_cnt505] = flat_adc_val;
		    //rec_buf[rec_cnt505] = 0x3FFF;
		    AdcCount++;
		    if (AdcCount == SPIKE_WIDTH) // overwrite 5 samples only
		    {
			isAdcRead = 0;
			AdcCount = 0;
			flat_adc_val = 0;
		...
		rec_offset++;
		remaining --;
		if (rec_offset==4096) rec_offset = 0;
	}
	#else
	vdmaptr = (64*1024 - inl(REG_VDMA_CBCR+edma_channel*0x100)) >> 1;
	while (  (((vdmaptr-rec_offset) & 0x3fff) > 16)  && (rec_cnt505<505) )
	{
		rec_buf[rec_cnt505] = rec_base[rec_offset];
		rec_cnt505++;
		rec_offset++;
		if (rec_offset==32*1024) rec_offset = 0;
	}
	#endif
	mod_timer(&adc_timer, jiffies + HZ/REC_SAMPLE);
	enable_irq(14);
	return;
}
#endif
...
//		outl((1<<IRQ_I2S), REG_AIC_MECR);
	...
	{
#ifdef AUDIOLOSS_DEBUG
		printk("Opening mic device1\n");
#endif
		minor = 1;	//dsp1
//		printk("dsp_record_openflag = %d\n", audio_dev.dsp_record_openflag);
...
#else
		#if defined(CONFIG_SPU_BUF_8K)
			audio_dev.record_buf_length = 8*1024;
		#else
	...
	#if defined(CONFIG_SPU_BUF_8K)
		#ifdef CONFIG_NORECINT
			for (ii=0; ii<4*1024; ii++)
			{
				rec_local = audio_dev.record_buf_addr+ii*2;
				*((unsigned short*)rec_local) = 0x7FFF;
			}
			rec_base = audio_dev.record_buf_addr;
			rec_cnt505 = 0;
			rec_offset = 0;
		#else
			memset(audio_dev.record_buf_addr, 0, 8*1024);
		#endif
	#else
		#ifdef CONFIG_NORECINT
...
//		printk("I2S recrod is ready !!!\n");
		...
		audio_dev.record_half_buf[0].ptr = 0;//FRAG_SIZE;
		audio_dev.record_half_buf[1].ptr = 0;//FRAG_SIZE;

#if 0  // remove adc_get_data
	#ifdef CONFIG_NORECINT
		init_timer(&adc_timer);
		adc_timer.function = adc_get_data;
		mod_timer(&adc_timer, jiffies + HZ/REC_SAMPLE);
	#endif
#endif
	}
			     ...
			     unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case SNDCTL_DSP_MIC_RESET_BUFFER:
		audio_dev.record_half_buf[0].ptr=audio_dev.record_half_buf[1].ptr=r_fragment_size;
		printk("MIC_RESET_BUFFER called\n");
		break;
...
#ifdef	CONFIG_REC_INTERP
		// stephen
		// memorize start pointer
		startdmabuf = inl(REG_VDMA_CBCR+edma_channel*0x100);
		startdmabuf = (8192 - startdmabuf);
		isAdcRead = 1;
		start_adc_offset = (startdmabuf + OFFSET_AFTER_ADC_READ*2) & 0x1FFF; // we will flat the value from 22 to 26 of the offset
		printk("GetAIN2: Start %d\n",startdmabuf);
...
//****************************************************************
		...
		case SNDCTL_DSP_GETISPACE:
			...
			info.fragsize = r_fragment_size/*FRAG_SIZE*/;
			info.fragstotal = 2;
			info.bytes = (r_fragment_size - audio_dev.record_half_buf[0].ptr) + (r_fragment_size - audio_dev.record_half_buf[1].ptr);
			if(info.bytes == 0)
				info.fragments = 0;
			else
			{
		...
		case RECBUF_GETALL:
			if (rec_cnt505==505)
			{
				if (copy_to_user((void *)arg, rec_buf, 505*2))
					return -EFAULT;
				rec_cnt505 = 0;
				return 505*2;
			}
		...
		case RECBUF_CLEAR:
		{
			rec_offset = (8192 - inl(REG_VDMA_CBCR+edma_channel*0x100))>>1;
			rec_cnt505 = 0;
			return 0;
		}
...
static void wb_audio_exit (void)
{
	...
	#if defined(CONFIG_SPU_BUF_8K)
		dma_free_writecombine(NULL, 8*1024, (dma_addr_t *)audio_dev.play_buf_addr , phaddrplay);
			...
			dma_free_writecombine(NULL, 8*1024, (dma_addr_t *)audio_dev.record_buf_addr , phaddrrecord);
		#endif
	#else
...
static int __init wb_audio_init(void)
{
	...
	#if defined(CONFIG_SPU_BUF_8K)
			dma_play_buf_addr = (unsigned int)dma_alloc_writecombine(NULL, (unsigned int)8*1024, (dma_addr_t *)&phaddrplay, GFP_KERNEL);

		#if defined(CONFIG_SOUND_W55FA93_RECORD_NAU8822) || \
			defined(CONFIG_SOUND_W55FA93_RECORD_NAU8501) || \
			defined(CONFIG_SOUND_W55FA93_RECORD_ADC)
			dma_rec_buf_addr = (unsigned int)dma_alloc_writecombine(NULL, (unsigned int)8*1024, (dma_addr_t *)&phaddrrecord, GFP_KERNEL);
		#endif
