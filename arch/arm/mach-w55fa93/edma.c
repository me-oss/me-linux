...
static struct w55fa93_edma_channel w55fa93_edma_channels[MAX_CHANNEL_NUM];
...
static struct w55fa93_edma_direction w55fa93_edma_set_dir[MAX_CHANNEL_NUM];
				...
				unsigned int dma_length)
{
	...
	do {
		...
		// set source and destination address
		psSGFmt->u32SourceAddr = virt_to_phys(u32SrcAddr);
		psSGFmt->u32DestAddr = u32DestAddr;
		// set stride transfer byte count & byte count
		psSGFmt->u32StrideAndByteCount = u32TranferByte;
		// set source offset byte length and destination offset byte length
...
//	__raw_writel(1 << channel, EDMA_DISR);
	ret = DrvEDMA_InstallCallBack(channel, interrupt, irq_handler, data);
...
#ifdef EDMA_USE_IRQ
	ret = request_irq(IRQ_EDMA, edma_irq_handler, SA_INTERRUPT, DRIVER_NAME, NULL);
	if (ret) {
...
//		DrvEDMA_DisableInt();
	...
	for (i = 0; i < MAX_CHANNEL_NUM; i++) {
		...
		DrvEDMA_EnableCH(i, eDRVEDMA_ENABLE);
#ifdef EDMA_USE_IRQ
		DrvEDMA_EnableInt(i, eDRVEDMA_SG | eDRVEDMA_BLKD | eDRVEDMA_TABORT);
#endif
	}
